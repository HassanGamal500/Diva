...
class AdminController extends Controller
{
	...
	public function dashboard(Request $request){
		...
		foreach($orders as $orders_data){
			...
			$orders_status_history = DB::table('orders_status_history')
				...
				->where('orders_id', '=', $orders_data->orders_id)
				->where('orders_status_description.language_id', '=', language())
		  ...
		  $recentProducts = DB::table('products')
			...
			->select('products.*', 'products_description.*', 'products_image')
		...
		$shipping_methods = DB::table('shipping_methods')->count();
		$result['shipping_methods'] = $shipping_methods;
		$returns = DB::table('returns')->count();
		$result['returns'] = $returns;
		$contacts = DB::table('contacts')->count();
		$result['contacts'] = $contacts;
		$top_user = DB::table('orders')
                ->join('users', 'users.id', '=', 'orders.customers_id')
                ->select(DB::raw('count(users.id) as user_count, users.id'))
                ->groupBy('users.id')
                ->orderBy('user_count', 'desc')
                ->take(5)
				->get();
		foreach($top_user as $user){
			$topFiveCustomer = DB::table('users')->where('id', '=', $user->id)->where('role_id', '=', null)->select('first_name', 'last_name')->first();
			$user->info = $topFiveCustomer;
		}
		$result['topUsers'] = $top_user;
		// dd($result['topUsers']);
	...
	public function addrole(Request $request){
		...
		if(count($roles)>0){
			...
			$brand_view = $roles[0]->brand_view;
			$brand_create = $roles[0]->brand_create;
			$brand_update = $roles[0]->brand_update;
			$brand_delete = $roles[0]->brand_delete;
			
			$designer_view = $roles[0]->designer_view;
			$designer_create = $roles[0]->designer_create;
			$designer_update = $roles[0]->designer_update;
			$designer_delete = $roles[0]->designer_delete;
			...
			$view_shipping = $roles[0]->view_shipping;
			$add_shipping = $roles[0]->add_shipping;
			$edit_shipping = $roles[0]->edit_shipping;
			$delete_shipping = $roles[0]->delete_shipping;
		...
		$result2[21]['link_name'] = 'Brands';
		$result2[21]['permissions'] = array(
					'0'=>array('name'=>'brand_view','value'=>$brand_view),
					'1'=>array('name'=>'brand_create','value'=>$brand_create),
					'2'=>array('name'=>'brand_update','value'=>$brand_update),
					'3'=>array('name'=>'brand_delete','value'=>$brand_delete),
					);

        $result2[22]['link_name'] = 'Designers';
		$result2[22]['permissions'] = array(
					'0'=>array('name'=>'designer_view','value'=>$designer_view),
					'1'=>array('name'=>'designer_create','value'=>$designer_create),
					'2'=>array('name'=>'designer_update','value'=>$designer_update),
					'3'=>array('name'=>'designer_delete','value'=>$designer_delete),
					);

		$result2[23]['link_name'] = 'categories';
		...
		$result2[24]['link_name'] = 'ShippingMethod';
		$result2[24]['permissions'] = array(
					'0'=>array('name'=>'view_shipping','value'=>$view_shipping),
					'1'=>array('name'=>'add_shipping','value'=>$add_shipping),
					'2'=>array('name'=>'edit_shipping','value'=>$edit_shipping),
					'3'=>array('name'=>'delete_shipping','value'=>$delete_shipping),
					);
	...
	public function addnewroles(Request $request){
		...
		$roles = DB::table('manage_role')->where('user_types_id',$request->user_types_id)->insert([
						...
						'brand_view' => $request->brand_view,
						'brand_create' => $request->brand_create,
						'brand_update' => $request->brand_update,
						'brand_delete' => $request->brand_delete,
						
						'designer_view' => $request->designer_view,
						'designer_create' => $request->designer_create,
						'designer_update' => $request->designer_update,
						'designer_delete' => $request->designer_delete,

						'view_shipping' => $request->view_shipping,
						'add_shipping' => $request->add_shipping,
						'edit_shipping' => $request->edit_shipping,
						'delete_shipping' => $request->delete_shipping,
