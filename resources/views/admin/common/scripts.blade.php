...
<script src="{!! asset('admin/js/mouse.js') !!}"></script>
...
{{--{{ URL::to('admin/deleteproductattributemodal')}}--}}
	...
	//deletebrandModal
	$(document).on('click', '#brandFrom', function(){
	...
	//deleteDesignerModal
	$(document).on('click', '#designerFrom', function(){
	...
	$(document).on('click', '#conditionFrom', function(){
	...
	$(document).on('click', '#shippingFrom', function(){
		var shipping_id = $(this).attr('shipping_id');
		$('#shipping_methods_id').val(shipping_id);
	...
	//deleteContactModal
	$(document).on('click', '#contactFrom', function(){
	...
	//deleteTeamModal
	$(document).on('click', '#deleteTeamId', function(){
	...
	//deletePartnerModal
	$(document).on('click', '#deletePartnerId', function(){
...
$('.mainCategory').on('change', function() {
	...
	$.ajax({
		...
		success: function(data){
			console.log(data);
			// $('.type_id').append(data);
			var append = "<option disabled selected>{{trans('labels.ChooseSubCategory')}}</option>";
			for (i = 0; i < data.length; i++) {
				append += "<option value='"+ data[i].id +"'>"+ data[i].name +"</option>"
			}
			$('.subCategory').html(append);
		}
	});
})
...
$('.purchase').on('change', function() {
    ...
    // 	console.log(id);
    if(id == 1){
        $('.display_insurance').css({'display':'unset'});
    } else {
        $('.display_insurance').css({'display':'none'});
    }
