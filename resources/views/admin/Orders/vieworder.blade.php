          ...
          <table class="table table-striped">
            ...
            <tr>
                ...
                @foreach($products->attribute as $attributes)
                  ...
                  {{-- <b>{{ trans('labels.Price') }}:</b> {{ $attributes->options_values_price }}   {{ $data['currency'][19]->value }}<br> --}}

                @endforeach</td>

                <td>{{ $products->final_price }}   {{ $data['currency'][19]->value }}</td>
             </tr>
            @endforeach
          ...
          <div class="table-responsive ">
            <table class="table order-table">
              <tr>
                ...
                <td>{{ $data['subtotal'] }}  {{ $data['currency'][19]->value }}</td>
              </tr>
              <tr>
                <th>{{ trans('labels.Tax') }}:</th>
                <td>{{ $data['orders_data'][0]->total_tax }}  {{ $data['currency'][19]->value }}</td>
              </tr>
              <tr>
                <th>{{ trans('labels.ShippingCost') }}:</th>
                <td>{{ $data['orders_data'][0]->shipping_cost }}  {{ $data['currency'][19]->value }}</td>
              </tr>
              @if(!empty($data['orders_data'][0]->coupon_code))
              <tr>
                <th>{{ trans('labels.DicountCoupon') }}:</th>
                <td>{{ $data['orders_data'][0]->coupon_amount }}  {{ $data['currency'][19]->value }}</td>
              </tr>
              @endif
              <tr>
                <th>{{ trans('labels.Total') }}:</th>
                <td>{{ $data['orders_data'][0]->order_price }}  {{ $data['currency'][19]->value }}</td>
              </tr>
            </table>
          </div>
