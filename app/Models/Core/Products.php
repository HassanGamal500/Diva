...
class Products extends Model
{
    ...
    public function paginator($request){
        ...
        $data = $this->sortable(['products_id'=>'DESC'])
            ...
            ->leftJoin('condition_description', 'condition_description.condition_id', '=', 'products.condition_id');
        ...
        $data->select('products.*', 'products_description.*', 'products.updated_at as productupdate', 'categories_description.categories_id',
        'categories_description.categories_name', 'condition_name')
    ...
    public function paginatorCustomer($request){
        $setting = new Setting();
        $myVarsetting = new SiteSettingController($setting);
        $commonsetting = $myVarsetting->commonsetting();
        $myVaralter = new AlertController($setting);
        ...
        $data = $this->sortable(['products_id'=>'DESC'])
            ->leftJoin('products_description', 'products_description.products_id', '=', 'products.products_id')
            ->leftJoin('condition_description', 'condition_description.condition_id', '=', 'products.condition_id')
            ->leftJoin('users', 'users.id', '=', 'products.customer_seller');
        ...
        $data->select('products.*', 'products_description.*', 'products.updated_at as productupdate', 'categories_description.categories_id',
        'categories_description.categories_name', 'condition_name', 'first_name', 'last_name')
            ->where('products_description.language_id', '=', $language_id)
            ->where('categories_description.language_id', '=', $language_id);
  ...
  public function insert($request){
    ...
    if ($request->hasFile('product_image')) {
        $product_image = Storage::disk('edit_path')->putFile('images/products', $request->file('product_image'));
    } else {
        $product_image = '';
    }
    

    // $sku = rand(0, 99999999999);
    // $checkSku = DB::table('products')->where('sku_code', '=', $sku)->count();
    // $x = 0;
    // // $sku = '';

    // while($checkSku > 0 && $x == 0) {
  ...
  public function edit($request){
    ...
    $subCategory = DB::table('products_to_categories')->where('products_id', '=', $products_id)->select('categories_id')->first();

    $mainCategory = DB::table('categories')->where('categories_id', '=', $subCategory->categories_id)->select('parent_id')->first();
    
    $result['subCategory'] = $subCategory->categories_id;
    $result['mainCategory'] = $mainCategory->parent_id;

    $result['categoriesSub'] = DB::table('categories')
    ->join('categories_description', 'categories_description.categories_id', '=', 'categories.categories_id')
    ->select('categories.categories_id as id', 'categories_name as name')
    ->where('parent_id', '=', $mainCategory->parent_id)
    ->where('language_id', '=', $language_id)
    ->get();
  ...
  public function updaterecord($request){
        ...
        //   }
        $getImage = DB::table('products')->where('products_id', '=', $request->id)->first();
        if ($request->hasFile('product_image')) {
  ...
  public function insertProductImages($request){
    $product_id = $request->products_id;
        ...
        if ($request->hasFile('product_image')) {
            $product_image = Storage::disk('edit_path')->putFile('images/products/images', $request->file('product_image'));
        } else {
            $product_image = '';
        }
        ...
        //     $img = DB::table('products_images')->insert(['image' => $imageName, 'products_id' => $product_id]);
        // }
  ...
  public function updateproductimage($request){
    ...
    $myVaralter = new AlertController($setting);
        ...
        //     $uploadImage = $request->oldImage;
        // }
    $getImage = DB::table('products_images')->where('products_id', '=', $request->products_id)->first();
  ...
  public function getOptionsValue($request){
    ...
    if($request->option_id == 4){
        ...
        ->select('products_options_values_id', 'products_options_values_size as options_values_name')
        ->get();
    } else {
        $value = DB::table('products_options_values')
        ->leftjoin('products_options_values_descriptions', 'products_options_values_descriptions.products_options_values_id', '=', 'products_options_values.products_options_values_id')
  ...
  public function displayProductConditions($request){
    $products_id = $request->id;
    $result['data'] = array('products_id'=>$products_id);
    $products_images = DB::table('products_images')
        ->LeftJoin('image_categories', function ($join) {
  ...
  public function addProductConditions($products_id){
    $result['data'] = array('products_id'=>$products_id);
    $products_images = DB::table('products_images')
        ->where('products_id','=', $products_id)
        ->orderBy('sort_order', 'ASC')
  ...
  public function insertProductConditions($request){
    $product_id = $request->products_id;
         if($request->image_id !== null){
             $image = $request->image_id;
         }else{
  ...
  public function editProductConditions($id){
  ...
  public function updateproductConditions($request){
    $setting = new Setting();
    $myVarsetting = new SiteSettingController($setting);
    $myVaralter = new AlertController($setting);
  ...
  public function deleteproductConditions($request){
        DB::table('products_images')
        ->where([
            'products_id' => $request->products_id,
            'id' => $request->id
