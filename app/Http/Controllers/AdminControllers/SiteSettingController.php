...
// slugify method
    ...
    public function aboutUs(Request $request){
        ...
        $title = array('pageTitle' => Lang::get("labels.aboutUs"));

        $result = array();

        $about = $this->Setting->aboutUs();
        ...
        return view("admin.settings.general.about",$title)->with('result', $result);

    }

    public function updateAbout(Request $request){
		$validator = validator()->make($request->all(), [
			'about_image' => 'nullable|image|mimes:jpeg,png,jpg,bmp,gif,svg',
		]);
		if ($validator->fails()) {
			$error = $validator->errors()->first();
			return redirect()->back()->withInput($request->all())->withErrors($error);
		}
		$title = array('pageTitle' => Lang::get("labels.EditAbout"));
		$exist = $this->Setting->updateAbout($request);
		return redirect()->back()->with('success', Lang::get("labels.AboutUpdatedMessage"));
	}
