...
class ShippingMethodsController extends Controller
{
    ...
    public function __construct(Shipping_method $shipping_method, Setting $setting, Languages $languages)
    {
      ...
      public function display(){
        ...
        $shippings = $this->Shipping_method->paginator(10);
        foreach($shippings as $shipping){
          $orders = DB::table('orders')->where('shipping_method', '=', $shipping->id)->count();
          $shipping['numOfOrders'] = $orders;
        }
        return view("admin.shipping.index")->with('shippings',$shippings);
      }
  
      public function add(Request $request){
        $title = array('pageTitle' => Lang::get("labels.AddShippingMethods"));
        $result['languages'] = $this->myVarsetting->getLanguages();
        return view("admin.shipping.add",$title)->with('result', $result);
      }
      ...
      public function insert(Request $request){
        ...
        $title = array('pageTitle' => Lang::get("labels.AddShippingMethods"));
        $this->Shipping_method->insert($request);
        return redirect()->back()->with('update', 'Content has been created successfully!');
      }
  
      public function edit(Request $request){
        $title = array('pageTitle' => Lang::get("labels.EditShipping"));
        $shipping_id = $request->id;
        $editShipping = $this->Shipping_method->edit($shipping_id);
        $result['languages'] = $this->myVarsetting->getLanguages();
        return view("admin.shipping.edit",$title)->with('editShipping', $editShipping)->with('result', $result);
      }
      ...
      public function update(Request $request){
        ...
        $title = array('pageTitle' => Lang::get("labels.EditShipping"));  
        $this->Shipping_method->updaterecord($request);
        return redirect()->back()->with('update', 'Content has been created successfully!');
      }
  
      public function delete(Request $request){
        $this->Shipping_method->destroyrecord($request);
