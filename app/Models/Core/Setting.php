...
class Setting extends Model
{
    ...
    public function fetchLanguages(){
        $language = DB::table('languages')
            ->leftJoin('language_description', 'language_description.languages_id', '=', 'languages.languages_id')
            ->select('languages.languages_id','language_description.name','languages.code', 'languages.directory','languages.is_default','languages.direction','languages.status','languages.sort_order')
    ...
    public function getunits($language_id){
       ...
       public function aboutUs(){
        ...
        public function updateAbout($request){
            $getImage = DB::table('about_us')->where('about_us_id', '=', $request->id)->first();
            if ($request->hasFile('about_image')) {
                $myFile = public_path('images/'.$getImage->about_image);
                File::delete($myFile);
                $about_image = Storage::disk('public')->putFile('about', $request->file('about_image'));
            } else {
                $about_image = $request->oldImage;
            }

            $about = DB::table('about_us')->where('about_us_id', $request->id)->update([
            ...
            foreach($languages as $language){
                DB::table('about_us_description')
                    ->where('about_us_id', '=', $request->id)
                    ->where('language_id', '=', $language->languages_id)
                    ->update([
                        'about_description_part'  		 =>   $request->about_description_part[$language->languages_id],
                        'about_description_full'   =>     $request->about_description_full[$language->languages_id],
                        'our_value'   =>     $request->our_value[$language->languages_id],
                        'mission'   =>     $request->mission[$language->languages_id],
                        'our_vision'   =>     $request->our_vision[$language->languages_id]
                    ]);
            }
        }
